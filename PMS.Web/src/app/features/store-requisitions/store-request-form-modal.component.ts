...
import { RequisitionApiService, CreateStoreRequestPayload, StockLinePayload } from '../../core/services/requisition-api.service';
...
@Component({
  selector: 'app-store-request-form-modal',
  standalone: true,
    ...
    <div class="modal-backdrop" (click)="close()">
      <div class="modal-panel modal-lg" (click)="$event.stopPropagation()">
        <div class="modal-header">
          <h3><app-icon name="file-text" [size]="20"></app-icon> New Store Requisition</h3>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="modal-body">
          <div class="form-grid">
            ...
            <div class="form-group">
              <label>Reason</label>
              <input formControlName="reason" class="form-control" placeholder="Purpose of request" />
            </div>
          </div>
...
export class StoreRequestFormModalComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const payload: CreateStoreRequestPayload = {
      requesterId: this.authStore.user()?.id || '',
      requestType: Number(v.requestType),
      reason: v.reason || undefined,
      details
    };
    this.api.createStoreRequest(payload).subscribe({
      next: () => { this.saving = false; this.saved.emit(); },
      error: () => { this.saving = false; this.notify.error('Error', 'Failed to create store request'); }
    });
  }
