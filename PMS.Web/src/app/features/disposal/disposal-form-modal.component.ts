...
import { DisposalApiService, CreateDisposalPayload } from '../../core/services/disposal-api.service';
...
const DISPOSAL_METHOD_LABELS: Record<number, string> = {
  ...
  [DisposalMethod.Tendering]: 'Tendering',
  [DisposalMethod.Scrapping]: 'Scrapping',
...
@Component({
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="modal-body">
          ...
          <div class="form-group">
            ...
            <select formControlName="shelfId" class="form-control">
              <option value="">N/A (Custody item)</option>
          ...
          <div class="form-group">
            ...
            <select formControlName="disposalMethod" class="form-control">
              <option *ngFor="let m of methodOptions" [value]="m.value">{{ m.label }}</option>
            </select>
          </div>
          <div class="form-group">
            <label>Notes / Justification</label>
            <textarea formControlName="notes" class="form-control" rows="3" placeholder="Committee decision, reason for disposal..."></textarea>
          </div>
  ...
  styles: [`
    ...
    .modal-panel { background: var(--bg-surface); border-radius: var(--radius-lg); box-shadow: var(--shadow-lg); width: 100%; max-width: 580px; }
...
export class DisposalFormModalComponent implements OnInit {
  ...
  conditionOptions = Object.entries(PROPERTY_CONDITION_LABELS).map(([v, l]) => ({ value: Number(v), label: l }));
  methodOptions = Object.entries(DISPOSAL_METHOD_LABELS).map(([v, l]) => ({ value: Number(v), label: l }));

  form: FormGroup = this.fb.group({
    itemId: ['', Validators.required],
    shelfId: [''],
    quantity: [1, [Validators.required, Validators.min(1)]],
    condition: [PropertyCondition.Damaged, Validators.required],
    disposalMethod: [DisposalMethod.Auction, Validators.required],
    notes: ['']
  });
  ...
  onSubmit(): void {
    ...
    const payload: CreateDisposalPayload = {
      itemId: v.itemId,
      shelfId: v.shelfId || undefined,
      quantity: Number(v.quantity),
      condition: Number(v.condition),
      disposalMethod: Number(v.disposalMethod),
      notes: v.notes || undefined
    };
    this.api.createDisposal(payload).subscribe({
      next: () => { this.saving = false; this.saved.emit(); },
      error: () => { this.saving = false; this.notify.error('Error', 'Failed to create disposal request'); }
    });
  }
